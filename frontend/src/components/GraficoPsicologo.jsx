...
function BarGraph({ data }) {
  ...
  const BW = 600, BH = 180, BPAD = { top: 10, right: 10, bottom: 30, left: 30 };
  ...
  return (
    <svg viewBox={`0 0 ${BW} ${BH}`} className="w-full" style={{ height: 180 }}>
      {[0, 25, 50, 75, 100].map(v => {
        ...
        return (
          <g key={v}>
            <line x1={BPAD.left} x2={BW - BPAD.right} y1={y} y2={y} stroke="#f0f0f0" strokeWidth={1} />
            <text x={BPAD.left - 4} y={y + 3} textAnchor="end" fontSize={9} fill="#aaa">{v}</text>
          </g>
        );
      })}
      {data.map((emp, ei) => {
        ...
        return (
          <g key={ei}>
            <text x={gx} y={BH - 6} textAnchor="middle" fontSize={9} fill="#aaa">{emp.name}</text>
            {METRICS.map((m, mi) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
        ...
        <p className="text-xs text-gray-400 mb-3">Média de todos os questionários respondidos</p>
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
        ...
        <p className="text-xs text-gray-400 mb-3">Últimas respostas do funcionário selecionado</p>
